...
use Devise\Mothership\Logs\Payload\Context;
use Devise\Mothership\Logs\Payload\Message;
use Devise\Mothership\Logs\Payload\Body;
use Devise\Mothership\Logs\Payload\Level;
use Devise\Mothership\Logs\Payload\Person;
use Devise\Mothership\Logs\Payload\Server;
use Devise\Mothership\Logs\Payload\Request;
use Devise\Mothership\Logs\Payload\Data;
use Devise\Mothership\Logs\Payload\Trace;
use Devise\Mothership\Logs\Payload\Frame;
use Devise\Mothership\Logs\Payload\TraceChain;
use Devise\Mothership\Logs\Payload\ExceptionInfo;
use Devise\Mothership\LogsDevise\Mothership\Logs;
use Devise\Mothership\Logs\Exceptions\PersonFuncException;
...
class DataBuilder implements DataBuilderInterface
{
     ...
     * @var string $errline
     *
     * @return Devise\Mothership\Logs\ErrorWrapper
     */
    ...
    private function stripShutdownFrames($backTrace)
    {
        foreach ($backTrace as $index => $frame) {
            ...
            $fatalHandlerMethod = (isset($method)
                                    && $method === 'Devise\Mothership\Logs\\Handlers\\FatalHandler::handle');
                                    
            $fatalHandlerClassAndFunction = (isset($class)
                                                && $class === 'Devise\Mothership\Logs\\Handlers\\FatalHandler'
                                                && isset($function)
                                                && $function === 'handle');
            
            $errorHandlerMethod = (isset($method)
                                    && $method === 'Devise\Mothership\Logs\\Handlers\\ErrorHandler::handle');
                                    
            $errorHandlerClassAndFunction = (isset($class)
                                                && $class === 'Devise\Mothership\Logs\\Handlers\\ErrorHandler'
                                                && isset($function)
                                                && $function === 'handle');
