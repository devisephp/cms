...
class DeviseTagTest extends \DeviseTestCase
{
	...
	{
		$tag = new DeviseTag(' data-devise="col[key], type, human name, group, category, alternate, defaults"');
  		...
  		assertEquals("alternate", $tag->alternateTarget);
  		assertEquals("defaults", $tag->defaults);
  		assertEquals(" data-devise=\"col[key], type, human name, group, category, alternate, defaults\"", $tag->value);
	}
	...
	{
		$tag = new DeviseTag("data-devise-<?php echo devise_tag_cid('key', 'field', null, 'key', 'type', 'human name', 'group', 'category', 'alternate', defaults) ?>");
  		assertEquals("key", $tag->id);
  		assertEquals("field", $tag->bindingType);
  		assertEquals(null, $tag->collection);
  		assertEquals("key", $tag->key);
  		...
  		assertEquals("alternate", $tag->alternateTarget);
  		assertEquals("defaults", $tag->defaults);
  		assertEquals("data-devise-<?php echo devise_tag_cid('key', 'field', null, 'key', 'type', 'human name', 'group', 'category', 'alternate', defaults) ?>", $tag->value);
	}
