<template>
  ...
  <div>
    ...
    <div id="devise-admin-content">
      <action-bar>
        <li class="dvs-btn dvs-btn-sm dvs-mb-2" :style="theme.actionButton" @click.prevent="showCreateForm()">
          Create New Release
        </li>
      </action-bar>
      ...
      <div v-for="release in pushedReleases" :key="release.id"
    ...
    <transition name="dvs-fade">
      <portal to="devise-root">
        <devise-modal class="dvs-z-50" v-if="showCreate" @close="showCreate = false">
          <h3 class="dvs-mb-4" :style="{color: theme.adminText.color}">Create New Release</h3>


          <div v-for="update in recentUpdates" :key="update.id"
               class="dvs-mb-6 dvs-items-center">

            <div class="dvs-w-full dvs-font-bold dvs-pr-8">
              <label><input type="checkbox"/> {{ update.model }}</label>
            </div>

            <div v-for="change in update.changes" :key="change.id"
          ...
          <button class="dvs-btn" @click="requestCreateRelease" :disabled="createInvalid"
                  :style="theme.actionButton">Create
          </button>
  ...
  export default {
    ...
    methods: {
      ...
      showForceRequiredMessage() {
        let force = confirm("You have uncommitted changes. Are you sure you want to create a release without committing your source?");
        if (force) {
