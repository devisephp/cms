...
<div class="dvs-bg-white dvs-text-black dvs-relative">
  <div v-if="imageToManage !== null" class="dvs-absolute dvs-absolute-center dvs-shadow-lg dvs-p-8 dvs-rounded dvs-z-50 dvs-bg-white">
    <h4 class="dvs-pb-4">Image Positioning:</h4>

    <div class="dvs-flex dvs-mb-4">
      <button class="dvs-btn dvs-mr-2" :style="theme.actionButton" @click="setImageFloat('left')">Float Left</button>
      <button class="dvs-btn dvs-mr-2" :style="theme.actionButton" @click="setImageFloat('')">No Float</button>
      <button class="dvs-btn dvs-mr-2" :style="theme.actionButton" @click="setImageFloat('right')">Float Right</button>
    </div>
...
export default {
  ...
  methods: {
    ...
    removeImage () {
      let newHTML = this.theEditor.el.trumbowyg('html').replace(this.imageToManage.currentTarget.outerHTML, '')
